...
"""Gmail API wrapper with OAuth — poll() for incoming, send() for outgoing."""
...
class EmailClient:
    ...
    def connect_imap(self) -> None:
        """Establish connection (named for backwards compat)."""
        self._connect()
    ...
    def poll(self, **kwargs) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            msg_refs = list(reversed(results.get("messages", [])))
            for ref in msg_refs:
                try:
    ...
    def _get_json_from_attachments(self, msg: dict):
        """Delegate to standalone function (backwards compat)."""
        return get_json_from_attachments(msg, self._service)

    def send(self, to_email: str, subject: str, body_dict: dict,
             attachment_filename: str = "payload.json") -> bool:
        ...
        try:
            ...
            protocol_logger.log_sent(
                email=to_email, message_type=message_type,
